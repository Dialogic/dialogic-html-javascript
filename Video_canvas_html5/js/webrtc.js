/*************************************************************
 *
 * Dialogic WebRTC Javascript library for PowerMedia XMS 2.2
 *
 * **********************************************************/ 
...
function Dialogic( ) {
  ...
  this.mediaOptions = { 'audio': {}, 'video': { 'width': 1280, 'height': 720, 'framerate': 30 } };
...
Dialogic.prototype.onChannelMessage = function( message ) {
  ...
  if ( message.type === "message" ) {
    ...
    if ( msg.type === 'rtc_room' ) {
      if ( this.guest ) {
        console.log("guest");
        wmsg = { type: "rtc_room", roomId: this.room };
        this.sendWebSocketMessage(wmsg);
      } else {
        this.room = msg.id;
      }
